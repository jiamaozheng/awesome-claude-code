...
async function fetchTextFileFromUrl(url: string): Promise<string | null> {
  try {
...
async function fetchTextFile(filePath: string): Promise<string | null> {
  const basePath = getBasePath();
  const localContent = await fetchTextFileFromUrl(
    `${basePath}data/content/${filePath}`
  );

  if (localContent !== null) return localContent;

  return fetchTextFileFromUrl(`${REPO_BASE_URL}/${filePath}`);
}
...
export async function fetchFileContent(
  ...
  try {
    ...
    if (content === null) throw new Error(`Failed to fetch ${filePath}`);
    return content;
  } catch (error) {
...
export async function downloadFile(filePath: string): Promise<boolean> {
  try {
    const content = await fetchTextFile(filePath);
    if (content === null) throw new Error("Failed to fetch file");
